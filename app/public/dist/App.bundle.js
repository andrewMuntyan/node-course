...
function autocomplete(input, latInput, lngInput) {
  ...
  var dropdown = new google.maps.places.Autocomplete(input); // eslint-disable-line
...
function ajaxHeart(e) {
  ...
  var heartBtn = this.heart;
  var flyingHeartClass = 'heart__button--float';
  _axios2.default.post(this.action).then(function (res) {
    var hearts = res.data.hearts;

    var isHearted = heartBtn.classList.toggle('heart__button--hearted');
    (0, _bling.$)('.heart-count').textContent = hearts.length;
    if (isHearted) {
      heartBtn.classList.add(flyingHeartClass);
      setTimeout(function () {
        return heartBtn.classList.remove(flyingHeartClass);
      }, 2500);
    }
...
async function setMapOptions() {
  var center = await getCurrentPosition();
  mapOptions = {
...
async function loadPlaces(map, lat, lng) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (response) {
    ...
    var markers = places.map(function (place) {
      ...
      var marker = new google.maps.Marker({ map: map, position: position }); // eslint-disable-line 
      marker.place = place;
      return marker;
    });
...
async function makeMap(mapDiv) {
  ...
  var autocomplete = new google.maps.places.Autocomplete(input); // eslint-disable-line 
...
function typeAhead(search) {
  ...
  searchInput.on('input', function (e) {
    ...
    _axios2.default.get('/api/search?q=' + searchText).then(function (_ref) {
      var _ref$data = _ref.data,
          stores = _ref$data === undefined ? [] : _ref$data;
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results for ' + searchText + '</div>');
    }).catch(function (err) {
...
(0, _autocomplete2.default)((0, _bling.$)('#address'), (0, _bling.$)('#lat'), (0, _bling.$)('#lng')); // eslint-disable-line
